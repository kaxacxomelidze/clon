...
const MIME: Record<string, string> = {
  ...
  '.woff':'font/woff', '.woff2':'font/woff2', '.ttf':'font/ttf', '.eot':'font/eot',
  '.json':'application/json', '.mp4':'video/mp4', '.webm':'video/webm',
  '.avif':'image/avif', '.pdf':'application/pdf',
};

// ─── Utilities ────────────────────────────────────────────────────────────────
function getCliArgs(): { cmd: string; args: string[] } {
...
function broadcast(job: Job, line: string) {
  job.lines.push(line);
  for (const res of job.listeners) sendSSE(res, line);
}

function serve(res: ServerResponse, body: string | Buffer, ct: string, status = 200) {
...
function listSites(baseDir: string): Array<{ hostname: string; pageCount: number }> {
  if (!existsSync(baseDir)) return [];
  try {
    return readdirSync(baseDir)
      .filter(name => {
        try { return statSync(join(baseDir, name)).isDirectory(); } catch { return false; }
      })
      .filter(name => existsSync(join(baseDir, name, 'route-map.json')))
      .map(name => {
        let pageCount = 0;
        try {
...
function outDirForHost(baseDir: string, hostname: string) {
    ...
    <div id="progress-overlay">
      ...
      <div id="pr-header">
        ...
        <div id="pr-meta">
          ...
          <span id="pr-elapsed-wrap" style="display:none">· <span id="pr-elapsed">0s</span> elapsed</span>
        </div>
      </div>
      <div class="pr-stats">
...
// ─── HTTP handler ─────────────────────────────────────────────────────────────
async function handle(req: IncomingMessage, res: ServerResponse, baseOutDir: string, port: number) {
  ...
  try {
    ...
    if (path === '/api/switch' && method === 'POST') {
      const { hostname } = JSON.parse(await readBody(req)) as { hostname: string };
    ...
    if (path === '/api/clone' && method === 'POST') {
      ...
      const job: Job = { id: jobId, status: 'running', lines: [], outputDir: outDir, hostname, listeners: new Set() };
      jobs.set(jobId, job);
      ...
      child.on('close', code => {
        if (code === 0) { job.status = 'done'; activeOutDir = outDir; broadcast(job, '[DONE]'); }
        else            { job.status = 'error'; broadcast(job, '[ERROR] Clone failed (exit ' + code + ')'); }
    ...
    if (path.startsWith('/api/cancel/') && method === 'POST') {
      ...
      if (job) { job.status = 'error'; for (const r of job.listeners) r.end(); job.listeners.clear(); }
      return json(res, { ok: true });
    }
    ...
    if (path === '/api/save' && method === 'POST') {
      const { route, html, host } = JSON.parse(await readBody(req)) as { route: string; html: string; host?: string };
      const dir = host ? outDirForHost(baseOutDir, host) : activeOutDir;
      const mapPath = join(dir, 'route-map.json');
      if (!dir || !existsSync(mapPath)) { res.writeHead(503); res.end('No active clone'); return; }
      const map = JSON.parse(readFileSync(mapPath, 'utf8')) as Record<string, string>;
    ...
    if (path === '/site' && method === 'GET') {
      ...
      const dir   = host ? outDirForHost(baseOutDir, host) : activeOutDir;
      const mapPath = join(dir, 'route-map.json');
      if (!dir || !existsSync(mapPath)) { res.writeHead(503); res.end('Not cloned yet'); return; }
      const map = JSON.parse(readFileSync(mapPath, 'utf8')) as Record<string, string>;
...
// ─── Start ────────────────────────────────────────────────────────────────────
export async function startServer(baseOutDir: string, port: number): Promise<void> {
  // Pre-load the most recently cloned site if one exists
