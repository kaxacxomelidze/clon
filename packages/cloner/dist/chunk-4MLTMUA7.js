...
function initLogger(outDir, verbose = false) {
...
var ABORT_PATTERNS = [
  ...
  // Segment (large, rarely needed)
  /\/log\?format=json&hasfast=true/,
  // YouTube-specific internal ping
  /\/generate_204/,
  // Chrome connectivity check
  /\/pagead\/lvz/,
  // Google Ads viewability ping
  /\/ccm\/collect/,
  // Google consent
  /\/api\/stats\/(qoe|ads|atr)/,
  // YouTube stats pings
  /\/api\/jnn\//,
...
var NAVIGATION_TIMEOUT = envInt("CLONYFY_NAVIGATION_TIMEOUT_MS", IS_SERVERLESS ? 25e3 : 3e4, 5e3, 12e4);
var ROUTE_FETCH_TIMEOUT = envInt("CLONYFY_ROUTE_FETCH_TIMEOUT_MS", IS_SERVERLESS ? 8e3 : 15e3, 1e3, 6e4);
...
async function capturePage(context, pageUrl, assetsDir) {
  ...
  await page.route("**/*", async (route) => {
    ...
    try {
      ...
      logger.debug(`  [FETCH FAIL] ${url}: ${err.message}`);
      await route.abort();
      return;
    }
    ...
    if ((isJson || isText) && resourceType !== "document") {
      try {
        body = (await response.text()).slice(0, 5e5);
      } catch {
      }
    }
    if (resourceType === "script") {
      const badScriptType = contentType.includes("text/html") || contentType.includes("application/json") || contentType.includes("text/xml") || contentType.includes("application/xml");
  ...
  try {
      ...
      if (!assetMap.has(u) && !shouldSkipAsset(u) && !ABORT_PATTERNS.some((p) => p.test(u))) {
        pendingAssets.push(async () => {
          try {
            ...
            try {
              failedAssets.add(new URL(u, pageUrl).href);
            } catch {
            }
          }
        });
    ...
    if (!IS_SERVERLESS) try {
      ...
      const clickTargets = await page.evaluate((selectors) => {
        const MAX = 10;
      ...
      if (clickTargets.length > 0) {
        ...
        await page.waitForLoadState("networkidle", { timeout: 6e3 }).catch(() => {
        });
      }
    } catch {
    }
    ...
    if (!IS_SERVERLESS && cmsJsonStubs.size > 0) {
      ...
      try {
        ...
        await page.waitForLoadState("networkidle", { timeout: 12e3 }).catch(() => {
        });
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
...
var envInt2 = (key, fallback, min, max) => {
  const raw = Number.parseInt(process.env[key] ?? "", 10);
  if (!Number.isFinite(raw)) return fallback;
  return Math.max(min, Math.min(max, raw));
};
...
var PAGE_CAPTURE_TIMEOUT = envInt2("CLONYFY_PAGE_CAPTURE_TIMEOUT_MS", IS_SERVERLESS2 ? 55e3 : 18e4, 1e4, 24e4);
var USER_AGENT3 = "Mozilla/5.0 (compatible; CLONYFY/0.1; +local archival)";
var STATIC_ASSET_LIMIT = IS_SERVERLESS2 ? 80 : 250;
var STATIC_ASSET_TIMEOUT = envInt2("CLONYFY_STATIC_ASSET_TIMEOUT_MS", IS_SERVERLESS2 ? 6e3 : 1e4, 1e3, 6e4);
var STATIC_PAGE_TIMEOUT = envInt2("CLONYFY_STATIC_PAGE_TIMEOUT_MS", IS_SERVERLESS2 ? 2e4 : 15e3, 3e3, 6e4);
...
var META_IMAGE_PROPERTIES = /* @__PURE__ */ new Set([
  "og:image",
  "og:image:url",
  "og:image:secure_url",
  "twitter:image",
...
var DYNAMIC_SEGMENT = /^([0-9a-f]{8,}|[0-9]+|[0-9a-f-]{36})$/i;
function normalizePath(pathname) {
  return pathname.split("/").map((seg) => DYNAMIC_SEGMENT.test(seg) && seg.length > 3 ? "[id]" : seg).join("/");
}
function fixtureKey(method, path) {
  return `${method.toUpperCase()}_${path.replace(/^\//, "").replace(/\//g, "_").replace(/\[|\]/g, "") || "root"}`;
}
function inferFieldsFromBody(raw) {
  if (!raw) return [];
  try {
...
function looksLikeForm(entries) {
  return entries.some(
    (e) => e.method === "POST" && !!e.postData && !e.url.includes("/graphql") && !e.url.includes("/gql") && !e.url.includes("/api/auth")
  );
}
...
function inferGraphQLOperation(postData) {
  ...
  try {
    ...
    return { operation: null, variables: null };
  }
}
function analyzeTraffic(entries, targetOrigin) {
  ...
  for (const [key, group] of groups) {
    ...
    const sampleRequest = inferredFields.length ? Object.fromEntries(inferredFields.map((f) => [f, ""])) : null;
    const gql = isGraphQL(first) ? inferGraphQLOperation(first.postData) : null;
    const finalSampleRequest = gql?.variables ? { variables: gql.variables, operationName: gql.operation } : sampleRequest;
    specs.push({
      method,
      path,
      ...
      isGraphQL: !!gql,
      graphQLOperation: gql?.operation ?? void 0
    });
  }
  return specs;
}
function tryParseJson(raw, contentType = "") {
  if (!raw) return null;
...
function toPrismaFields(fields) {
  const reserved = /* @__PURE__ */ new Set(["id", "createdAt"]);
  return fields.filter((f) => !reserved.has(f)).map((f) => `  ${f.replace(/[^a-zA-Z0-9_]/g, "_")}  String  @default("")`).join("\n");
}
...
Total unique assets saved: ${uniqueAssets.size}`);
    ...
    const allConsoleErrors = uniqueRecords.flatMap(
      (r) => r.network.filter((n) => n.method === "CONSOLE_ERROR").map((n) => ({ url: r.url, body: n.body }))
    );
...
//# sourceMappingURL=chunk-4MLTMUA7.js.map